	...
	<div id="container">
		<section id="boardview">
			...
			<table class="tbl_write">
				...
				<tr>
					<td><input type="text" name="userId" value="${board.userId}"
						readonly></td>
				</tr>
				<tr>
					<td><textarea rows="5" cols="60" name="boardContent" readonly>${board.boardContent}</textarea>
					</td>
				</tr>
				<tr>
					<td>조회수:${board.hit}</td>
			  ...
			  수정일:<fmt:formatDate value="${board.updatedTime}"
				...
				<c:if test="${board.userId  eq sessionId}">
					...
					<a href="/board/delete?id=${board.id}"
						onclick="return confirm('정말로삭제하시겠습니까?')"><button>삭제</button></a>
				</c:if>
				...
				<form action="/reply/insert" method="post" id="replyform"
					...
					<p>
						<input type="hidden" name="replyer" value="${sessionId}"
							id="replyer"> 작성자:${sessionId}
					</p>

					<p>
						<textarea rows="2" cols="50" placeholder="댓글작성" id="replyContent"
							name="replyContent"></textarea>
					</p>
					<button type="button" value="등록" onclick="replyinsert()">댓글등록</button>
				</form>
			</c:if>

			<c:if test="${empty sessionId}">
				<div class="replylogin">
					<a href="/user/login"><i class="fa-solid fa-user"></i>로그인한 사용자만
						댓글등록이 가능합니다.</a>
				</div>
			</c:if>
			<!-- 댓글목록 -->
	 ...
	 const replyinsert = function(){
		...
		$.ajax({
			...
			success:function(replyList){
				...
				document.getElementById("replyContent").value =""; //댓글내용초기화
			},
			error:function(){
				console.log("댓글등록실패");
			}
